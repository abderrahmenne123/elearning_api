...
router.get('/',verifToken,UserController.getAllUsers) ; // verifTOken permet de verifier si un utilisateur est authentifier ou non , // isAdmin verfier si un utilisateur est l'admin avant d'exectuer la tache principale 
router.get('/:role',verifToken,UserController.getUsersByType); 
router.delete('/:id',verifToken,isAdmin,UserController.deleteUser); 
router.put('/assign/:id',verifToken,UserController.assignPermisstion); 
